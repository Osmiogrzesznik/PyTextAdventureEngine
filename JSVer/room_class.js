...
if 'add' in ev:
    add = ev['add']
for typ in add: #have effect on accessible room data#['exits', 'item_containers', 'items', 'objects', 'switches', 'ds', 'links']
obToAddToTyp = add[typ]
self.dct[typ][obToAddToTyp['name']] = obToAddToTyp#
for example add item to items# problem with exits - they are objects and stored in the array#
return {
    't': ev['t']
}#
...
if 'change' in ev: #TODO think about generalising this logic, #
    if ev['evtypname'
}
is instance of list loop through it with the
function as teh paramater#
else execute
function on ev['evtypname'] itself
changes = ev['change']
...
results.append(result)# BOOKMARK trying to finish use functionality

# should
return false or success and text reason why or text# feedback on success
return ev#
let game logic to handle player given items, statuses
...
if not propName in ob:
    m = 'cannot find {} prop in {} object'.format(
        propName, objName)
self.ui.out(m)
raise Exception(m)# check on source data just in
    case ofnot mutable stuff
...
property_to_change = ob[propName]# it doesnt work with returned objects ? because there is no pointers in ppython# Solution : change source data just in
    case ofnot mutable stuff
...
if not cmd:
    ...
    if 'do' in cmd: #do serves as handy synonym
    for
    other commands# do is
    for
    weird cases only 'do'
commands should lead to effects themselves
return cmd['do']# !TODO all below(add, addtoDesc)# should be factored out to Effect and executed always when any action results in effect on room# add and addToDesc may happen always and independently of outcome
if 'add' in cmd: #this should be onExec or onSpecCmd RoomEvent
add = cmd['add']
for typ in add: #should i have some way to determine
if event works on player or Room or just run it on both and see ? #
    if typ == 'eventClass': #eventClass = add[typ]#
if eventClass == 'room':

    #have effect on accessible room data#['exits', 'item_containers', 'items', 'objects', 'switches', 'ds', 'links']
obToAddToTyp = add[typ]
self.dct[typ][obToAddToTyp['name']] = obToAddToTyp#
for example add item to items# problem with exits - they are objects and stored in the array#
return text feedback on what happened
...
if len(objName.split()) > 1:
    self.ui.out('multiword!!! TODO')# TODO when searching
for typ in typs:
    if typ in self.dct:
...
return False# TODO no object in data, game - init or this
function should check
if the word is mentioned in room description or other descriptions and say that it is "not interesting"

def getDescription(self, objName):
    obj = self.getObject(objName)
if obj: #there should be check canDescribe
if 'd' in obj:
    return obj['d']
elif 'noDesc' in self.dct:
    return self.dct['noDesc']
else :#each room should had its own
default none response#
if no description of object but it exists
return "just normal " + objName
else :#WAS: return 'nothing interesting about that or it doesnt exist here'#
LEAD TO BUG: couldn 't determine in game logic if '
around ' exists and default to Room desc
return False#
let game logic handle that

def getObjectForCatg(self, cat, objName, retkey = False):
    if not cat in self.dct:
    return False
ret = byKeyOrSubAlt(self.dct[cat], objName, False, retkey)# ret may have ob prop but here doesnt matter
if ret:

    return ret# each room should had its own
default none response
elif 'noObj' in self.dct:
    ret = self.dct['noObj']# should be string only
else :
    ret = False# what about alts ?

    def enterFrom(self, entrance = False) :
    if not self.was_entered:
    self.ui.out(self.once)
self.was_entered = True# TODO onEnter event resetting ghost in ActionCastle duncgeons
...
if entrance:
    self.entrance = entrance# self.ui.out(self.name)

# self.ui.out("      0: back to location " + entrance.name)# for exno, ex in enumerate(self.exits): #self.ui.out("       " + str(exno + 1) + ":" + #ex.name + " DEBUG: " + str(ex.dict))
return self
